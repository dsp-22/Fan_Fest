...
$my_id = $is_guest ? 0 : $_SESSION['user_id'];
...
$friend_status = null;
if (!$is_me && !$is_guest) {
    $cf = $conn->prepare("SELECT id, status FROM friends WHERE ((user_id_1 = ? AND user_id_2 = ?) OR (user_id_1 = ? AND user_id_2 = ?))");
    $cf->bind_param("iiii", $my_id, $profile_id, $profile_id, $my_id);
    $cf->execute();
    $cf_row = $cf->get_result()->fetch_assoc();
    if ($cf_row) {
        $friend_status = $cf_row['status'];
        $is_friend = ($cf_row['status'] === 'accepted');
    }
} else {
    $is_friend = true;
}
...
if (!$is_me && !$is_guest && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'remove_friend') {
        $stmt = $conn->prepare("DELETE FROM friends WHERE (user_id_1 = ? AND user_id_2 = ?) OR (user_id_1 = ? AND user_id_2 = ?)");
        $stmt->bind_param("iiii", $my_id, $profile_id, $profile_id, $my_id);
        $stmt->execute();
        $stmt->close();
        header("Location: profile.php?id=$profile_id");
        exit();
...
if ($is_guest && $is_me) {
    ...
    $my_theme = $my_settings['theme_color'] ?? 'blue';
}
...
$active_gradient = $theme_map[$my_theme]  ?? $theme_map['blue'];
$accent          = $accent_map[$my_theme] ?? '#0984e3';
$light           = $light_map[$my_theme]  ?? '#e8f4fd';

// handle settings save
if ($is_me && !$is_guest && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_profile') {
...
// user data
if ($profile_id == 0) {
    // Populate fake profile data for the guest so the UI still looks nice
    $user = [
        'first_name' => 'Guest',
        'last_name' => 'Fan',
        'username' => 'guest_fan',
        'bio' => 'Create an account to customize your profile, unlock badges, and connect with friends!',
        'profile_pic' => 'default_avatar.png',
        'favorite_team' => 'Not set',
        'seat_location' => '',
        'location_public' => 0
    ];
    $points = 0; $friend_count = 0; $order_count = 0; $rank = '-';
    $activities = [];
} else {
...
// load leaderboard for everyone so guests can see it
...
// next rsvp — only show to self or friends
$next_rsvp = null;
if (($is_me || $is_friend) && !$is_guest) {
    $nr = $conn->prepare("SELECT e.event_name, e.event_date, e.location_name FROM rsvps r JOIN events e ON e.event_id = r.event_id WHERE r.user_id = ? AND DATE(e.event_date) >= CURDATE() ORDER BY e.event_date ASC LIMIT 1");
...
if ($is_me && !$is_guest) {
    ...
    $ord = $conn->prepare("SELECT 'order' as type, CONCAT('Order #', order_id) as title, CONCAT('\$', total_price) as subtitle, created_at as ts FROM orders WHERE user_id = ? ORDER BY created_at DESC LIMIT 4");
    ...
    $rv = $conn->prepare("SELECT 'rsvp' as type, e.event_name as title, e.location_name as subtitle, NOW() as ts FROM rsvps r JOIN events e ON e.event_id = r.event_id WHERE r.user_id = ? ORDER BY e.event_date DESC LIMIT 4");
    $rv->bind_param("i", $profile_id);
    $rv->execute();
    $rv_res = $rv->get_result();
    while ($row = $rvres->fetch_assoc()) $activities[] = $row;
    $rv->close();
...
$type_meta = [
    'checkin' => ['icon' => '📍', 'bg' => $light,   'pts' => true,  'href' => 'checkin.php'],
    'order'   => ['icon' => '🌭', 'bg' => '#fff7ed', 'pts' => false, 'href' => 'past_orders.php'],
    'rsvp'    => ['icon' => '🎟️', 'bg' => '#fdf4ff', 'pts' => false, 'href' => 'checkin.php'],
    'friend'  => ['icon' => '👥', 'bg' => '#f0fdf4', 'pts' => false, 'href' => 'friends.php'],
];
?>
...
<style>
    ...
    .btn-add       { background: <?php echo $accent; ?>; color: white; }
    .btn-friends   { background: #f1f2f6; color: #636e72; border: 2px solid #dfe6e9; }
    .btn-pending   { background: #fff7ed; color: #f97316; border: 2px solid #fed7aa; }
    .btn-remove    { background: white; color: #d63031; border: 2px solid #ff7675; }
    ...
    .stat-card:hover { border-color: <?php echo $accent; ?>; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,0.09); }
    ...
    .about-card { background: white; border-radius: 16px; box-shadow: 0 6px 20px rgba(0,0,0,0.06); overflow: hidden; }
    ...
    .tab-card { background: white; border-radius: 16px; box-shadow: 0 6px 20px rgba(0,0,0,0.06); overflow: hidden; display: flex; flex-direction: column; }
    ...
    .activity:hover { background: #f8f9fa; border-radius: 8px; padding-left: 6px; padding-right: 6px; }
    ...
    .not-friends-panel .nf-icon { font-size: 48px; margin-bottom: 16px; }
    .not-friends-panel h3 { font-size: 16px; font-weight: 800; color: #2d3436; margin: 0 0 8px; }
    .not-friends-panel p  { font-size: 13px; color: #b2bec3; margin: 0 0 24px; }
...
<div class="main-container">
    ...
    <div class="hud">
        <div class="hud-avatar">
            <?php if (!empty($user['profile_pic']) && $user['profile_pic'] !== 'default_avatar.png' && file_exists($user['profile_pic'])): ?>
        ...
        <div class="hud-info">
            ...
            <div class="hud-meta">
                ...
                <span class="h-pill"><?php echo $friend_count; ?> Friends</span>
        ...
        <div class="hud-btns">
            <?php if ($is_me && $is_guest): ?>
                <a href="register.php" class="btn btn-settings">Create Account</a>
            <?php elseif ($is_me): ?>
            ...
            <?php elseif ($friend_status === 'accepted'): ?>
                <form method="POST">
                    ...
                    <button type="submit" class="btn btn-remove">Remove Friend</button>
                </form>
    ...
    <div class="stats-row">
        ...
        <?php else: ?>
            ...
            <div class="stat-card">
                <div class="stat-val"><?php echo $current_milestone; ?></div>
                <div class="stat-lbl">Milestone</div>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="profile-grid">
        ...
        <div class="about-card" id="aboutCard">
            ...
            <div class="info-item">
                <div class="info-lbl">Games Attended</div>
                <div class="info-val"><?php echo $points; ?></div>
            </div>

            <?php if ($is_me || $is_friend): ?>
                <div class="info-item">
        ...
        <div class="tab-card" id="tabCard">
                    ...
                    <div class="tab-panel-inner">
                        ...
                        <?php else: ?>
                            <?php foreach ($activities as $a):
                                $meta = $type_meta[$a['type']] ?? ['icon' => '•', 'bg' => '#f1f2f6', 'pts' => false, 'href' => '#'];
                            ?>
                            <a href="<?php echo $meta['href']; ?>" class="activity">
            ...
            <?php elseif ($is_friend): ?>
                <div class="tab-bar">
                    <button class="tab-btn active" onclick="switchTab(this,'activity')">Recent Games</button>
                </div>
                <div class="tab-panel active" id="activity">
                    <div class="tab-panel-inner">
                        <?php
                        $ci2 = $conn->prepare("SELECT e.event_name as title, e.location_name as subtitle, c.checkin_time as ts FROM checkins c JOIN events e ON e.event_id = c.event_id WHERE c.user_id = ? ORDER BY c.checkin_time DESC LIMIT 6");
                        $ci2->bind_param("i", $profile_id);
                        $ci2->execute();
                        $ci2_res = $ci2->get_result();
                        $has_rows = false;
                        while ($row = $ci2_res->fetch_assoc()):
                            $has_rows = true;
                        ?>
                        <div class="activity" style="text-decoration:none; color:inherit;">
                            <div class="act-icon" style="background:<?php echo $light; ?>;">📍</div>
                            <div>
                                <div class="act-title"><?php echo htmlspecialchars($row['title']); ?></div>
                                <div class="act-sub"><?php echo htmlspecialchars($row['subtitle']); ?> · <?php echo date("M j, Y", strtotime($row['ts'])); ?></div>
                            </div>
                        </div>
                        <?php endwhile; $ci2->close(); ?>
                        <?php if (!$has_rows): ?>
                            <div class="act-empty">No check-ins yet.</div>
                        <?php endif; ?>
                    </div>
                </div>

            <?php else: ?>
                <div class="not-friends-panel">
                    <div class="nf-icon">🔒</div>
                    <h3>Friends Only</h3>
                    <p>Add <?php echo htmlspecialchars($user['first_name']); ?> as a friend to see their activity.</p>
                    <?php if ($friend_status === 'pending'): ?>
                        <button class="btn btn-pending" disabled>⏳ Request Sent</button>
                    <?php else: ?>
                        <?php if ($is_guest): ?>
                            <button type="button" class="btn btn-add" onclick="requireLogin('add friends')">+ Add Friend</button>
                        <?php else: ?>
...
function matchHeight() {
    ...
    panels.forEach(p => { p.style.maxHeight = Math.max(targetH, 300) + 'px'; });
}
