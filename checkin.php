...
// --- TIMEZONE FIX ---
// Force the app to operate in Eastern Time to prevent UTC rollover
...
$date_limit = $is_valid_search
    ? date('Y-m-d 23:59:59', strtotime('+10 days'))
...
if (!$is_valid_search) $sql .= " LIMIT 20";

$events_result = $conn->query($sql);

// --- TRENDING POPULAR EVENTS QUERY ---
$popular_query = "
    SELECT e.event_id, e.event_name, e.location_name, e.league, COUNT(c.checkin_id) as total_attendance
    FROM checkins c
...
// --- DYNAMIC VENUE QUERY (HYBRID APPROACH) ---
$db_venues = [];
try {
    // Attempt to pull dynamic venues if the lat/lon columns exist
    ...
    // Silently ignore if columns aren't there yet
}

// Fallback to extended hardcoded array if DB pull is empty
if (empty($db_venues)) {
    $db_venues = [
        ['name' => 'Franklin Hall', 'lat' => 39.1673, 'lon' => -86.5233],
        ['name' => 'Simon Skjodt Assembly Hall', 'lat' => 39.1809, 'lon' => -86.5222],
        ['name' => 'Memorial Stadium', 'lat' => 39.1809, 'lon' => -86.5256],
        ['name' => 'United Center', 'lat' => 41.8806, 'lon' => -87.6742],
        ['name' => 'Soldier Field', 'lat' => 41.8623, 'lon' => -87.6167],
        ['name' => 'Wrigley Field', 'lat' => 41.9484, 'lon' => -87.6553],
        ['name' => 'Lucas Oil Stadium', 'lat' => 39.7601, 'lon' => -86.1639],
        ['name' => 'Lincoln Financial Field', 'lat' => 39.9008, 'lon' => -75.1675],
        ['name' => 'Citizens Bank Park', 'lat' => 39.9061, 'lon' => -75.1665],
        ['name' => 'Wells Fargo Center', 'lat' => 39.9012, 'lon' => -75.1720]
    ];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .game-name-link { cursor:pointer; color:#1a1a2e; font-weight:700; transition:color 0.15s; }
        .game-name-link:hover { color:<?php echo $accent; ?>; }
        .av-stack { display:flex; align-items:center; margin-top:5px; }
        .av-stack .av-sm { width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:8px; font-weight:700; margin-left:-5px; border:2px solid white; overflow:hidden; flex-shrink:0; }
        .av-stack .av-sm:first-child { margin-left:0; }
        .av-stack .av-sm img { width:100%; height:100%; object-fit:cover; }
        .av-stack-label { font-size:11px; color:#888; margin-left:7px; }
        .action-cell { text-align:right; padding-right:15px; white-space:nowrap; }
        .action-cell form { display:block; margin-bottom:6px; }
        .action-cell form:last-child { margin-bottom:0; }
        .btn-rsvp { width:110px; padding:8px 0; border-radius:5px; cursor:pointer; font-size:0.85em; font-weight:600; }
        ...
        .btn-checkin  { width:110px; padding:8px 0; border:none; border-radius:5px; cursor:pointer; font-size:0.85em; font-weight:600; background:<?php echo $accent; ?>; color:white; }
        .btn-attended { width:110px; padding:8px 0; border:none; border-radius:5px; font-size:0.85em; font-weight:600; background:#ccc; color:#666; cursor:not-allowed; }
...
<div class="main-container">
    ...
    <div class="compact-card" id="venue-card" style="display:none;">
        <h3 style="margin:0 0 10px; font-size:1.1em; color:#333;">Venues Nearby</h3>
        <div id="venue-list" style="display:flex; gap:10px; flex-wrap:wrap;"></div>
    </div>
    ...
    <div class="card">
        <h2>Upcoming Games <?php echo $is_valid_search ? '(Search Results)' : '(Today & Tomorrow)'; ?></h2>
        <?php if ($events_result && $events_result->num_rows > 0): ?>
            <table style="width:100%;">
                ...
                ?>
                <tr style="border-bottom:1px solid #eee; <?php echo $done ? 'opacity:0.6; background:#f9f9f9;' : ''; ?>">
                    <td style="padding:15px;">
                        <span class="game-name-link" onclick="openGameModal(<?php echo $row['event_id']; ?>)">
                            <?php echo htmlspecialchars($row['event_name']); ?>
                        </span>
                        <?php if($done) echo " <span style='color:green;font-size:0.8em;margin-left:10px;'>Checked In</span>"; ?>
                        <br>
                        <small style="color:#666;">
                            <a href="maps.php?venue=<?php echo urlencode($row['location_name']); ?>" style="color:<?php echo $accent; ?>;text-decoration:none;font-weight:600;display:inline-flex;align-items:center;gap:5px;">
                                <span>📍</span><?php echo htmlspecialchars($row['location_name'] ?? 'TBA'); ?>
                            </a>
                            | <?php echo date("M j, g:i A", strtotime($row['event_date'])); ?>
                        </small>
                        ...
                        <?php elseif ($row['rsvp_count'] > 0): ?>
                            <br><small style="color:#888;font-size:0.75em;"><?php echo $row['rsvp_count']; ?> fan<?php echo $row['rsvp_count']!=1?'s':''; ?> going</small>
                        <?php endif; ?>
                    </td>
                ...
                <tr style="border-bottom:1px solid #eee;">
                    <td style="padding:12px 15px;">
                        ...
                        <small style="color:#666;">
                            ...
                            <span style="margin-left:8px;background:#f1f2f6;border-radius:10px;padding:2px 8px;font-size:0.85em;"><?php echo htmlspecialchars($rsvp_row['league'] ?? ''); ?></span>
                        </small>
                    </td>
    ...
    <div class="card">
        ...
        <?php if ($history_result->num_rows > 0): ?>
            <div style="display:flex;gap:15px;flex-wrap:wrap;margin-top:15px;">
                ...
                <a href="event_details.php?id=<?php echo $ach['event_id']; ?>" style="display:block;text-decoration:none;color:inherit;background:#f1f2f6;padding:15px;border-radius:12px;text-align:center;width:140px;border:1px solid #eee;">
...
<script>
// Call our new PHP endpoint to get location-specific data
